...
def fitness(obj):
    ...
    score = Game.scoreSumHeight(sn2d)
    return score
...
def mutate(intensity, obj):
    minimum = 1
    maximum = min(5 if obj.getOrder() < 10 else int(obj.getOrder()/2)+3, 12)
    ...
    for i in xrange(ui):
        ri = r.randint(0,3)
        if ri == 0:
            ab = GR.randomAddBond(graph, 3.0, r)
            if ab is not None:
                GR.addBondToGraph(graph, ab[0], ab[1], 1200, 8000)
        elif ri == 1:
            GR.randomDelBond(graph, r)
        elif ri == 2:
            ab = GR.randomAddParticle(graph, .5, 1.0, r)
            if ab is not None:
...
termc = GeneAlg.make_criteria(60*1, None)
